    ...
    <el-container>
      <el-header>
        <div class="header-content">
          ...
          <h1 style="margin: 0; margin-left: 16px;">{{ directorName || '导演详情' }}</h1>
        </div>
      </el-header>
...
const directorId = computed(() => parseInt(route.params.id));

const loading = ref(true);
const movies = ref([]);
const total = ref(0);
const pageKey = computed(() => `director_${directorId.value}`);
...
const loadMoviesRaw = async () => {
  ...
  if (!directorId.value) {
    ElMessage.error('无效的导演ID');
    loading.value = false;
    return;
  }
  
  const result = await window.electronAPI.directors.getMovies(directorId.value, {
  ...
  if (result.success) {
    ...
    if (result.director && result.director.name) {
      directorName.value = result.director.name;
    } else if (directorId.value) {
      directorName.value = `导演 ${directorId.value}`;
    }

    ...
    <el-container>
      <el-header>
        <div class="header-content">
          ...
          <h1 style="margin: 0; margin-left: 16px;">{{ actorName || (viewMode === 'folder' ? '文件目录详情' : '女优详情') }}</h1>
        </div>
      </el-header>
...
// 优先使用 query 参数中的 viewMode，如果没有则根据 id 类型判断
const viewMode = computed(() => {
  if (route.query.viewMode) {
    return route.query.viewMode;
  }
  // 如果没有指定 viewMode，根据 id 类型判断
  // 如果 id 是数字，使用女优目录模式；否则使用文件目录模式
  const id = route.params.id;
  const parsedId = parseInt(id);
  return isNaN(parsedId) ? 'folder' : 'actor';
});

const actorId = computed(() => {
  const id = route.params.id;
  console.log('ActorDetail - 路由参数id:', id, '类型:', typeof id, '模式:', viewMode.value);
  // 文件目录模式：id是文件夹名（字符串）
...
const movies = ref([]);
const total = ref(0);
const pageKey = computed(() => `actor_${actorId.value}`);
...
const displayViewMode = ref('thumbnail'); // 显示模式：文字/缩图
const actorName = ref('');
...
const loadMoviesRaw = async () => {
  ...
  if (!actorId.value) {
    console.error('ActorDetail - actorId.value 为空:', actorId.value);
  ...
  console.log('ActorDetail - 调用 actors:getMovies, id:', id, '类型:', typeof id);
  
  const result = await window.electronAPI.actors.getMovies(id, {
  ...
  console.log('ActorDetail - actors:getMovies 返回结果:', result);
  if (result.success) {
    ...
    if (result.actor && result.actor.name) {
      ...
      // 如果没有返回actor信息，根据模式设置标题
      if (viewMode.value === 'folder' && actorId.value) {
        // 文件目录模式：使用文件夹名
        actorName.value = typeof actorId.value === 'string' ? actorId.value : String(actorId.value);
      } else if (viewMode.value === 'actor' && actorId.value) {
        // 女优目录模式：尝试从 actors:getById 获取
        try {
          const actorResult = await window.electronAPI.actors.getById(actorId.value, { viewMode: viewMode.value });
          if (actorResult.success && actorResult.data && actorResult.data.name) {
            actorName.value = actorResult.data.name;
          } else {
            actorName.value = `演员 ${actorId.value}`;
          }
        } catch (error) {
    ...
    console.log('ActorDetail - 设置actorName:', actorName.value);
